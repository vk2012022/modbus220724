...
def update_register_values():
    ...
    global first_connection, read_first_group
    try:
        logger.debug("Обновление значений регистров")

        def update_register_label_and_entry(address, count, label, entry, delay=0):
            root.after(delay, lambda: read_and_update(address, count, label, entry))

        def read_and_update(address, count, label, entry):
        ...
        # Первая группа регистров
        if first_connection or read_first_group:
            logger.debug("Чтение регистров 1 группы")
            update_register_label_and_entry(0, 2, label_cycle_time_value, entry_cycle_time, 0)
            update_register_label_and_entry(2, 2, label_full_cycle_time_value, entry_full_cycle_time, 100)
            update_register_label_and_entry(4, 2, label_kp_value, entry_kp, 200)
            update_register_label_and_entry(6, 2, label_ki_value, entry_ki, 300)
            update_register_label_and_entry(8, 2, label_kd_value, entry_kd, 400)
            update_register_label_and_entry(10, 2, label_dead_zone_value, entry_dead_zone, 500)
            update_register_label_and_entry(18, 2, label_tx1_value, entry_tx1, 600)
            update_register_label_and_entry(20, 2, label_ty1_value, entry_ty1, 700)
            update_register_label_and_entry(22, 2, label_tx2_value, entry_tx2, 800)
            update_register_label_and_entry(24, 2, label_ty2_value, entry_ty2, 900)
            update_register_label_and_entry(26, 2, label_tx1_value2, entry_tx1_2, 1000)
            update_register_label_and_entry(28, 2, label_ty1_value2, entry_ty1_2, 1100)
            update_register_label_and_entry(30, 2, label_tx2_value2, entry_tx2_2, 1200)
            update_register_label_and_entry(32, 2, label_ty2_value2, entry_ty2_2, 1300)
            update_register_label_and_entry(38, 2, label_cycle_time_value2, entry_cycle_time2, 1400)
            update_register_label_and_entry(40, 2, label_full_cycle_time_value2, entry_full_cycle_time2, 1500)
            update_register_label_and_entry(42, 2, label_kp_value2, entry_kp2, 1600)
            update_register_label_and_entry(44, 2, label_ki_value2, entry_ki2, 1700)
            update_register_label_and_entry(46, 2, label_kd_value2, entry_kd2, 1800)
            update_register_label_and_entry(48, 2, label_dead_zone_value2, entry_dead_zone2, 1900)
            update_register_label_and_entry(50, 2, label_boiler_on_temp_value, entry_boiler_on_temp, 2000)
            update_register_label_and_entry(52, 2, label_boiler_off_temp_value, entry_boiler_off_temp, 2100)
            update_register_label_and_entry(56, 2, label_home_sensor_2_value, None, 2200)
            update_register_label_and_entry(58, 2, label_security_sensor_value, None, 2300)
            update_register_label_and_entry(60, 2, label_radiator_supply_sensor_value, None, 2400)
            update_register_label_and_entry(62, 2, label_floor_supply_sensor_value, None, 2500)

            read_first_group = False

        # Вторая группа регистров
        logger.debug("Чтение регистров 2 группы")
        update_register_label_and_entry(12, 2, label_current_temp_value, None, 0)
        update_register_label_and_entry(14, 2, label_required_temp_value, None, 150)
        update_register_label_and_entry(16, 2, label_outdoor_temp_value, None, 300)
        update_register_label_and_entry(34, 2, label_required_temp_value2, None, 450)
        update_register_label_and_entry(36, 2, label_current_temp_value2, None, 600)
        update_register_label_and_entry(54, 2, label_boiler_temp_value, None, 750)

        root.after(3500, update_register_values)  # Обновление каждые 3 секунды

        if first_connection:
            root.after(2500, disable_first_connection)

    except Exception as e:
        logger.error(f"Ошибка при обновлении значений регистров: {e}")

def disable_first_connection():
...
def set_read_first_group():
    """Устанавливает флаг для однократного чтения регистров первой группы."""
    global read_first_group
    read_first_group = True
...
def write_input_values():
    ...
    try:
        ...
        set_read_first_group()  # Устанавливаем флаг для однократного чтения первой группы
