...
# Глобальные переменные для отслеживания первого подключения и необходимости чтения первой группы регистров
    ...
    try:
        ...
        def update_register_label_and_entry(address, count, label, entry):
            ...
            if registers:
                ...
                logger.debug(f"Адрес {address}: значение {value}")  # Отладочный вывод значения
        ...
        update_register_label_and_entry(14, 2, label_required_temp_value2, None)  # Требуемая температура контура 2
