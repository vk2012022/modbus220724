...
# Добавление новой кнопки для записи только во вновь созданные регистры для второго контура
...
button_boiler_auto = tk.Button(label_frame_boiler, text="АВТО ГВС", command=lambda: toggle_flag(11), bg="grey",
                               width=20, height=2)
button_boiler_auto.grid(row=1, column=0, pady=5)

label_boiler_relay = tk.Canvas(label_frame_boiler, width=40, height=40, bg="grey")
label_boiler_relay.grid(row=1, column=1, pady=5, padx=10)

label_boiler_on_temp = tk.Label(label_frame_boiler, text="ТЕМП ВКЛ НАГРЕВА ГВС:", font=("Arial", 12))
label_boiler_on_temp.grid(row=2, column=0, pady=5, sticky="e")
label_boiler_on_temp_value = tk.Label(label_frame_boiler, text="0.0", font=("Arial", 12))
label_boiler_on_temp_value.grid(row=2, column=1, pady=5, sticky="w")
entry_boiler_on_temp = tk.Entry(label_frame_boiler, font=("Arial", 12))
entry_boiler_on_temp.grid(row=2, column=2, pady=5, padx=10)

label_boiler_off_temp = tk.Label(label_frame_boiler, text="ТЕМП ВЫКЛ НАГРЕВА ГВС:", font=("Arial", 12))
label_boiler_off_temp.grid(row=3, column=0, pady=5, sticky="e")
label_boiler_off_temp_value = tk.Label(label_frame_boiler, text="0.0", font=("Arial", 12))
label_boiler_off_temp_value.grid(row=3, column=1, pady=5, sticky="w")
entry_boiler_off_temp = tk.Entry(label_frame_boiler, font=("Arial", 12))
entry_boiler_off_temp.grid(row=3, column=2, pady=5, padx=10)

button_write_boiler = tk.Button(label_frame_boiler, text="ЗАПИСЬ ГВС", command=write_boiler_values, bg="blue",
                                fg="white", width=20, height=2)
button_write_boiler.grid(row=4, column=0, columnspan=3, pady=5)


# Инициализация состояний флагов и цветов кнопок при первом подключении
def initialize_flags():
    try:
        ...
        logger.error(f"Ошибка при инициализации флагов: {e}")
...
update_labels()  # Начальное обновление меток
