...
def update_register_values():
    ...
    try:
        ...
        if first_connection or read_first_group:
            ...
            # Выставляем задержку 7 секунд перед следующим обновлением регистров
            root.after(7000, lambda: root.after(3000, update_register_values))
            read_first_group = False  # Сброс флага после чтения первой группы
            ...
            # Выставляем задержку 3 секунды перед следующим обновлением регистров


            root.after(3000, update_register_values)  # Обновление каждые 3 секунды
